...
``` sh
storm jar target/test-1.0-SNAPSHOT.jar test.CrawlTopology -conf crawler-conf.yaml -local
```
...
``` sh
storm jar target/test-1.0-SNAPSHOT.jar  org.apache.storm.flux.Flux --local crawler.flux --sleep 86400000
```
